...
class Embedding(object):
    def __init__(self):
        path = '/home/tootiya/CoMAE-main/codes/metric/word2vec' # please modify the path manually
...
def eval_emb_metrics(hypothesis, references, emb=None, metrics_to_omit=None):
    ...
    for hyp in hypothesis:
        ...
        embs_non_none = [emb for emb in embs if emb is not None]
        if len(embs_non_none) == 0:
            avg_emb = None
        else:
            avg_emb = np.sum(embs_non_none, axis=0) / np.linalg.norm(np.sum(embs_non_none, axis=0))
